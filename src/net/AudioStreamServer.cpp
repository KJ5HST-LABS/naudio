...
private:
    ...
    // (docs/audio-streaming-protocol-v1.md:409-410 and the constants table at :774-775), so
...
void AudioStreamServer::rejectClient(const std::shared_ptr<ClientConnection>& connection,
    ...
    //
    // REJECT_DRAIN_BUDGET_MS is a CEILING, not a cost: the ordinary call finds the request
    // buffered, or arriving within a few milliseconds, reads it, and exits on the next read's 2 ms
    // timeout. The drain waits THROUGH the quiet before the first byte (issue #104): the request is
    // sent by a peer thread that must be scheduled after its connect() returns, and on a loaded
    // windows-latest that took longer than the 2 ms a first empty read allowed, so the close landed
    // on the request and reset the peer — the #87 failure again, one attempt in ~fifty. Only a
    // peer that sends nothing pays the whole budget. It is bounded at all because this runs on the
    // accept thread, where time spent is time no other client is admitted — and that bound is
    // pinned by ASilentPeerCannotPinTheAcceptThreadPastTheDrainBudget, which is the only arm that
    // exercises it: every other silent peer in the suite closes its own socket first.
    connection->discardPendingInput(REJECT_DRAIN_BUDGET_MS);
