...
static NA_TEST_ATOMIC int g_tx_ok = 0;  /* set when the signature reaches the server's TX callback */
 ...
 * NA_TEST_ATOMIC (c_atomic_compat.h), NOT `volatile`. This comment used to say volatile plus the 20 ms nanosleep barriers
...
static NA_TEST_ATOMIC int g_rx_ok = 0;          /* set when KNOWN arrives at the RX audio callback   */
static NA_TEST_ATOMIC int g_connected = 0;      /* set by on_connected                               */
static NA_TEST_ATOMIC int g_roster_count = -1;  /* last count seen via on_clients_update             */
