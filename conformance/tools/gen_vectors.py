...
# with a 0-skipped gate, so extended-form vectors could not enter it before the reference
# implementation encoded them. Since issue #91 B3, Conformance.GoldenVectorsV12 loads the
# separate file under its own fail-closed, 0-skipped gate.
OUT_V12 = sys.argv[2] if len(sys.argv) > 2 else os.path.join(
    os.path.dirname(OUT) or ".", "vectors-v1_2.ini")
# Spec 1.4 (SS6.8, discovery) gets its own file for exactly the same reason 1.2 did, and
# keeping them separate is what lets vectors.ini stay byte-identical across every spec
# revision that only ADDS control types.
OUT_V14 = sys.argv[3] if len(sys.argv) > 3 else os.path.join(
    os.path.dirname(OUT) or ".", "vectors-v1_4.ini")
...
# ---- spec 1.4 (SS6.8) discovery encoders ----

def ctrl_discover(token) -> bytes:
    # 0x60 | token u32 BE. A probe carries no identity: it is sent to a broadcast
    # address, so anything in it is readable by everyone on the segment.
    return bytes([0x60]) + struct.pack('>I', token)

def ctrl_discover_reply(token, port, transports, rate, bits, ch, clients, maxc, name) -> bytes:
    # 0x61 | token u32 | port u16 | transports u8 | rate u32 | bits u8 | ch u8
    #      | clients u8 | max u8 | nameLen u8 | name
    # The name is clamped to 255 BYTES, identically to ControlMessage::discoverReply --
    # a 256-byte name behind a u8 prefix would wrap to 0 and mis-frame the reply, and
    # both implementations must produce the same bytes for an over-long name.
    nb = name.encode('utf-8')[:255]
    return (bytes([0x61])
            + struct.pack('>IHB', token, port, transports)
            + struct.pack('>IBBBB', rate, bits, ch, clients, maxc)
            + bytes([len(nb)]) + nb)
...
# ---- spec 1.2 vectors (§6.2.1) -- SEPARATE FILE, its own 0-skipped gate (loaded since B3) ----
#
...
# ---- spec 1.4 vectors (SS6.8) -- SEPARATE file, its own 0-skipped gate --------------------

rec14("discover-probe", kind="control_v14", controlType="0x60", token="3735928559",
      expectedPayloadHex=h(ctrl_discover(0xDEADBEEF)))

rec14("discover-reply-named", kind="control_v14", controlType="0x61", token="3735928559",
      port="4533", transports="3", sampleRate="48000", bitsPerSample="16", channels="2",
      clientCount="1", maxClients="4", serverName="shack",
      expectedPayloadHex=h(ctrl_discover_reply(0xDEADBEEF, 4533, 0x03, 48000, 16, 2, 1, 4,
                                               "shack")))

# The minimal legal form: 16 fixed bytes and no name. A prober that cannot handle an
# empty label would fail here rather than in the field.
rec14("discover-reply-unnamed", kind="control_v14", controlType="0x61", token="0",
      port="4533", transports="2", sampleRate="48000", bitsPerSample="16", channels="2",
      clientCount="0", maxClients="4", serverName="",
      expectedPayloadHex=h(ctrl_discover_reply(0, 4533, 0x02, 48000, 16, 2, 0, 4, "")))

# A FULL server still answers (SS6.8 item 3), and says so: clientCount == maxClients.
# Occupancy is a fact the chooser needs, not a reason for silence.
rec14("discover-reply-full", kind="control_v14", controlType="0x61", token="1",
      port="4533", transports="2", sampleRate="12000", bitsPerSample="16", channels="1",
      clientCount="4", maxClients="4", serverName="full",
      expectedPayloadHex=h(ctrl_discover_reply(1, 4533, 0x02, 12000, 16, 1, 4, 4, "full")))
...
with open(OUT_V12, "w") as f:
    ...
    f.write("# GENERATED by tools/gen_vectors.py. Loaded by Conformance.GoldenVectorsV12\n")
    f.write("# under the same fail-closed, 0-skipped gate as vectors.ini (loaded together\n")
    f.write("# with the reference implementation at issue #91 B3).\n")
...
with open(OUT_V14, "w") as f:
    f.write("# net-audio audio streaming protocol -- spec 1.4 (SS6.8, discovery) golden vectors\n")
    f.write("# GENERATED by tools/gen_vectors.py. Loaded by Conformance.GoldenVectorsV14\n")
    f.write("# under the same fail-closed, 0-skipped gate as vectors.ini and vectors-v1_2.ini.\n")
    f.write("# kind=control_v14: DISCOVER / DISCOVER_REPLY encode vectors.\n")
    f.write("# See conformance/README.md for the per-kind field schema.\n\n")
    for name, kv in records_v14:
        f.write("[%s]\n" % name)
        for k, v in kv.items():
            f.write("%s = %s\n" % (k, v))
        f.write("\n")
...
print("wrote %d spec-1.4 vectors to %s" % (len(records_v14), OUT_V14))
