...
speakers. The same small program as the [C example](../c), compiled as C++: it uses
only the public `naudio.h` C ABI — it never names a naudio C++ type and links only the
shared `naudio` library. naudio *does* ship a native C++ class (`AudioStreamClient`), but
this example deliberately does **not** use it: the point is that the flat C ABI is
self-sufficient from C++ too, not just from plain C.
...
```sh
cmake -S . -B build
cmake --build build --target na_cpp_play_to_speakers
```

The program is `build/examples/na_cpp_play_to_speakers`.
...
build/examples/na_cpp_play_to_speakers --port 4533           # another: plays it on your speakers
```

**Choose an output device:**

```sh
build/examples/na_cpp_play_to_speakers --list-devices        # list output device ids
build/examples/na_cpp_play_to_speakers --playback-id 2 --host 192.168.1.10 --port 4533
```
...
```sh
build/examples/na_cpp_play_to_speakers --backend null --port 4533 --seconds 5
```
...
It is the same source logic. Only the things C++ requires differ:

- `<stdatomic.h>` → `<atomic>` (`std::atomic<long long>` / `std::atomic<int>`, with
  `.fetch_add()` / `.load()` / `.store()`).
- The callbacks handed to the C ABI are declared `extern "C"`, which gives them C language
  linkage so their addresses are assignable to the ABI's function-pointer fields — the
  correct, portable way to pass C++ functions to a C API.

Everything else — the CLI, the device selection, the `na_client_*` call sequence — is identical.
```
