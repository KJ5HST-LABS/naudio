...
// from C++ — it never names a naudio C++ type and links only the shared `naudio`
// library — so it is a worked example of driving the C ABI from a C++ program.
// (naudio also ships a native C++ class, AudioStreamClient; this example
// deliberately does NOT use it — the point is that the flat C ABI is sufficient
// from C++ too, the same way it is from plain C.)
//
...
#include <atomic>  // std::atomic — the C++ counterpart of the C client's <stdatomic.h>

#include "naudio.h"  // the only audio header — the C ABI, #included from C++
...
struct rx_stats {
    ...
    std::atomic<long long> nonzero_bytes{0};  // a frame of underrun silence is all-zero; this proves content arrived
    std::atomic<int>       have_first{0};
    char                   first_hex[24] = {0};  // first up-to-8 bytes of the first frame, hex (a wire fingerprint)
};

// ---- callbacks handed to the C ABI as function pointers ----------------------------------
// extern "C" gives these C language linkage, so their addresses are assignable to the C ABI's
// function-pointer fields — the correct, portable way to hand C++ functions to a C API.
extern "C" {
...
static void on_rx_audio(const unsigned char* pcm, size_t n, void* user) {
    rx_stats* s = static_cast<rx_stats*>(user);
    s->frames.fetch_add(1);
    s->bytes.fetch_add((long long)n);
    long long nz = 0;
    for (size_t i = 0; i < n; i++)
        if (pcm[i] != 0) nz++;
    s->nonzero_bytes.fetch_add(nz);
    if (!s->have_first.load() && n > 0) {
        size_t k = n < 8 ? n : 8;
        for (size_t i = 0; i < k; i++)
            sprintf(s->first_hex + i * 2, "%02x", pcm[i]);
        s->first_hex[k * 2] = '\0';
        s->have_first.store(1);
    }
}
...
int main(int argc, char** argv) {
    ...
    rx_stats stats;  // all counters zero-initialized via the in-class initializers above
    ...
    const long long rx_frames = stats.frames.load();
    const long long rx_bytes  = stats.bytes.load();
    const long long rx_nonzero = stats.nonzero_bytes.load();
    ...
    fprintf(stderr,
            ...
            avg_bps, elapsed, stats.have_first.load() ? stats.first_hex : "(none)",
    ...
    printf("RESULT status=%s connected=%s rx_frames=%lld rx_bytes=%lld nonzero_bytes=%lld first_frame_hex=%s avg_bps=%lld\n",
           ...
           stats.have_first.load() ? stats.first_hex : "", avg_bps);
