...
[`c/inject_tone.c`](c/inject_tone.c) — `na_c_inject_tone`, the sending client: it injects a 1 kHz sawtooth as TX audio at the server's format from the hardware-free `null` backend, so the TX path is proved with no consuming application in the loop (see [`c/README.md`](c/README.md)). Built with the examples, not shipped.
