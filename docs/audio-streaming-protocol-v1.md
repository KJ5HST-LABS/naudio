...
**Spec version:** 1.4 (2026-09-01) — **discovery is adopted from §13.4** as the new normative §6.8, via the §11 minor-extension path: two new control types (`DISCOVER` `0x60`, `DISCOVER_REPLY` `0x61`) carried in ordinary `CONTROL` frames. The framing, the type enum (0x00–0x04), the CRC semantics and every pre-existing golden vector are **untouched** — a pre-1.4 endpoint ignores both new types, which is exactly what §11 requires of it. The exchange is **client-initiated and side-effect-free**: a server answers a probe without creating a connection, consuming a client slot or starting a stream, and it **never** transmits unsolicited. A server also listens on a **rendezvous port** (§2.5) independently of the transport it serves audio over, and its reply carries the port it actually serves on — so discovery resolves an address **and** a port, and a TCP-only server is discoverable despite having no datagram path of its own. See §6.8.
...
- **Discovery uses a rendezvous port**, `4533` by default — the same number as the default service port, because a client that knows nothing else knows this one. A server listens for `DISCOVER` there *in addition to* answering probes aimed at the port it serves audio on, and the reply carries that service port (§6.8). So a client that knows only the rendezvous number finds a server serving on any port, and one that already knows the service port may probe it directly. Two consequences worth stating: only **one server per host** can hold a given rendezvous port, so where several run the first to start wins it and the rest are found only by a probe aimed at their own service port; and there is still **no beacon** — a server never transmits unsolicited.
...
6. A server SHOULD **also** listen for `DISCOVER` on a **rendezvous port** (§2.5), independently of the transport it serves audio over, and answer with the port it actually serves on. This is what makes discovery resolve a port rather than only an address, and it is the **only** way a **TCP-only** server can be discovered at all: TCP has no datagram path to an unknown sender, so such a server can never answer through its own transport. The rendezvous listener is a UDP socket whatever the server serves audio over — TCP and UDP port spaces are independent, so a TCP server serving on the rendezvous port number still binds it on UDP. A DUAL server reports both bits in `transports`.
7. Binding the rendezvous port is **best effort**. A UDP server already serving on that port cannot bind it a second time and does not need to, because its transport already answers probes aimed there; and only one server per host can hold it, so where several run the first to start wins. A server that fails to bind it is still discoverable by a probe aimed at its own service port, and MUST NOT treat the failure as fatal.
...
| Discovery rendezvous port (spec 1.4, §2.5) | 4533 (UDP), same number as the default service port |
