...
### Added
- **On Windows the service runs with no console window, logs to a file, and has a tray icon.** The daemon is a console program, so the logon task and the Start-menu shortcut opened a console window at every login — the window was the only log, and closing it ended the service. The packages now carry `na_audio_daemonw.exe` beside `na_audio_daemon.exe`: the same program built for the Windows GUI subsystem, the way `pythonw.exe` stands beside `python.exe`. It is what the logon task and the shortcut run. Launched with nothing to print to, it writes to `%LOCALAPPDATA%\naudio\daemon.log` — where a configuration-file refusal is named by file and line, as the launchd agent's log names it on a Mac — and it puts an icon in the notification area: right-click for *Open control page* and *Quit Network Audio Service*. A double-click on the `.exe` itself opens the page, as a double-click on the macOS app does. The console build stays for the terminal and is what every test runs. A Windows *service* proper was considered again and is still the wrong shape: services run in session 0, which has no audio devices. Opening the page and reading the logon task's switch no longer go through `cmd.exe`, which under the windowed build would have flashed a console window once a second. The release pipeline reads both builds' PE subsystem field, runs the windowed one bare and requires its usage, its refusals and its control page to reach the log, and reports what the tray did on the runner.
- **naudio has an icon, and every launcher shows it** (issue #103). On macOS `Network Audio Service.app` carries it, so the Finder, the Dock while the page opens, Login Items and the one-time background-activity notification all show it instead of the generic grid; the bundle's Developer ID seal covers it, and the installer gate proves the placed file is the sealed one. On Windows it is compiled into `na_audio_daemon.exe`, so Explorer, the taskbar, the **Network Audio Service** Start-menu shortcut and Add/Remove Programs show it, and the setup and uninstall programs carry it as their own. On Linux the desktop entry's `Icon=naudio` resolves through the installed hicolor theme at nine sizes instead of borrowing the theme's `audio-card`. The artwork lives under `packaging/icons/` with the script that derives the three containers from it.
- **The control page shows whether the service runs at login, and on a Mac opens Login Items** (issue #102, third part). A *Runs at login* row beside the autostart checkbox reads *On*, *Off* or *Not installed* from the platform's own switch on every poll — on macOS the agent's presence in the user's launchd domain, which is what the Login Items switch adds and removes; on Linux the user unit and `systemctl --user is-enabled`; on Windows the logon task and its Disabled state — so a switch flipped in System Settings shows on the page within a second. The row mirrors the switch and does not offer one: nothing outside System Settings can move the macOS switch (measured), so the honest control is the row's **Open Login Items…** button, served by `POST /api/open-login-items` behind the same three browser defences as every other action. On Linux the row names the `systemctl --user` command; on Windows, the task. `/api/state` carries the reading as `service` for anything else that polls it. The quit dialog's "start it again" sentence names Network Audio Service and Login Items on macOS.

### Fixed
- **A client turned away by a full server on Windows always learns why** (issue #104). The reject is decided at accept, before the client's `CONNECT_REQUEST` can have arrived, and the drain that #87 added before the close gave up on its first empty 2 ms read; when the request landed a few milliseconds later it hit a closed socket, the stack answered RST, and Windows discarded the client's unread reject — one attempt in about thirty on a loaded machine, and deterministically for a client that sends 10 ms after connecting. The drain now waits for the request, up to a 50 ms budget that only a silent peer pays in full. Measured on windows-latest across three lanes of sixty runs: the old drain lost a reject in a third of runs and failed the late-sender arm every time; the new one lost none in 7,200 attempts. macOS and Linux were never affected (their stacks keep queued data readable after a reset). The rc9 packages carry the fix; this entry was written after the tag was cut.
- **A capture that delivers nothing but silence at full rate is called silent, and the daemon says what to check** (issue #105, first part). A process whose Microphone permission macOS has denied is handed zeros at full rate and no error: the daemon captured megabytes of them, reported `pipeline: PASS`, and the control page showed a green *running* with both meters at −120 dBFS and nothing to explain it (measured). Now the daemon watches for exactly that shape — frames arriving, every sample at the meter's floor, on both channels, for three seconds — and says so once: a flushed `[capture] silence:` line in the log while the run is still going, a `silence` line after the signal verdict in the `capture-probe` and `hardware` summaries (not a gate; a quiet band is still a pass), and on the control page an amber notice above the running stream, from `captureSilentMs` and `silenceHint` in `/api/state`. The sentence is the daemon's, not the page's, because the likely cause is the platform's: on macOS the *Privacy & Security › Microphone* row for Network Audio Service; anywhere, a muted input or the wrong device. A stalled capture (no frames) is not called silent — that shows in the delivered percentage and is a different failure — and any signal resets the watch.
- **On macOS the Microphone prompt and the Privacy & Security row name the service, not a file** (issue #105, second part). macOS attributes a Microphone request to a process's application bundle and to nothing else; the background service ran `/usr/local/bin/na_audio_daemon`, so the prompt and the pane read `na_audio_daemon` with a generic icon, and the usage sentence in the app's Info.plist was never shown because the app was not the process asking. An Info.plist embedded in the executable was tried first and changed neither (measured on macOS 27). So the service now runs the copy of the daemon that is `Network Audio Service.app`'s own executable — the launcher script that used to exec `/usr/local/bin/na_audio_daemon` is gone, the daemon is installed a second time as the bundle's executable, and the launchd agent's program is `/Applications/Network Audio Service.app/Contents/MacOS/na_audio_daemon`. Measured from a signed package: the prompt reads *Allow "Network Audio Service" to access your microphone?* over the sentence *Network Audio Service streams the audio your radio's USB audio interface receives to your digital-mode software.*, and the stream then runs from that process. Two consequences for a Mac upgraded from an earlier release: the Microphone answer given to `na_audio_daemon` does not carry over, so the next Start asks once more under the service's name (and the old row can be removed from the pane); and launchd keeps the old program path until the next login or a flip of the Login Items switch. The app is part of the service now, not just a shortcut to its page, and must stay in `/Applications`. Opening the app still opens the page: launched with no arguments from inside a bundle the daemon takes the launcher's options (control mode, no time limit, open the page); with any argument it is the command line it always was. macOS can also fail the very first open while its question is on screen rather than wait for the answer; the error now says to press Start again after answering. `INSTALL.md` and the man page gain a section on the permission, on what a refusal looks like (silence, not an error), and on where to change the answer. The release pipeline signs the bundle with the capture entitlements and the hardened runtime around the daemon copy and requires the agent to run the bundle's own executable, signed under the bundle's identifier — the key macOS holds the grant under. A signed `.pkg` carries that sealed bundle whole, as a zip the postinstall extracts, rather than its executable as a loose file: the notary validates every binary in a payload on its own, and a bundle's executable is valid only inside its bundle (its signature binds the bundle's `Info.plist`) — the first notarization after this change refused the package over exactly that file, and the same copy run outside a bundle is killed by the kernel. An unsigned build still assembles the bundle from the pieces.
- **A device chosen on the control page follows the device, not its number** (issue #107). The picker wrote only the device's backend id, and an id is a position in PortAudio's enumeration at that moment: a USB microphone unplugged before the next login moves every id after it down one, and the saved `capture-id = 1` then opened the laptop's microphone as "the radio" — with no diagnostic, since a microphone is a microphone (measured, with the speakers saved as the sink feeding the room into itself). The page now writes the device's name beside its id, and the daemon lets the name decide: among the devices a pattern matches, one whose name equals it beats one that merely contains it, the saved id breaks a tie only while its occupant is still one of those matches (two identical codecs), and a name that matches nothing refuses to start — naming the pattern and what now holds the id — instead of opening it. This is a change for a file, or a command line, that carries *both* `capture`/`--capture` and `capture-id`/`--capture-id`: the id used to win silently. A file with the id alone opens it as given, as before; the page adds the name on the next save. `/api/state` and `/api/devices` carry `resolved` — what each saved choice resolves to against the current list, and whether it moved — computed by the same function the stream uses at start, so the picker shows the device the stream would open, says when it has moved (nothing to do) and when it is not connected, and Start stays off naming it. The start-up log line names the device *and* the rule that chose it. The playback choice is resolved the same way. `INSTALL.md` and the man page describe the choice as by name.

### Changed
- **naudio describes itself as what it is — a generic network-audio toolkit — and the Hamlib bridge as its one domain-specific component.** The README opened with the project's amateur-radio origin, the installers and the control page spoke of "the radio's USB audio interface" and "your digital-mode software", the Microphone prompt promised to stream "the audio your radio's USB audio interface receives to your digital-mode software", the daemon's `--help` captured "the radio RX", `na_wav_tap` counted in FT8 periods and named WSJT-X, the getting-started guide had a section "For radio amateurs", the C ABI header addressed "a C / Hamlib-based consumer" and explained the duplex mode as "not PTT" in terms of carriers and CAT, and the Linux desktop entry filed the page under *HamRadio*. Every one of those now says the generic thing: a capture device, the applications that listen, a USB audio interface that macOS calls a microphone, a 15-second boundary, a WAV another program can analyse, a C consumer, a mode that switches nothing on at the far end. The bridge keeps every reference it had — it is the radio-specific component, and INSTALL.md, the README and the getting-started guide still say what it is, why it is not in the packages and where its documentation lives; the end-to-end verification procedure is linked as what it is. Identifiers do not move: flags, the `USB Audio CODEC` default pattern, `na_hamlib_bridge`, and the `ft8` reliability profile (`NA_RELIABILITY_UDP_FT8` is a public ABI name, so renaming it is an ABI decision, not a wording one — its comments now describe what it is tuned for).
...
- **The service is Network Audio Service on Windows and Linux too — the Start-menu shortcut, the applications-menu entry, the logon task's description and the user unit's description carry the name the macOS app already had.** One service had two names: macOS showed *Network Audio Service* in Applications, Login Items and the Microphone prompt (issues #102 and #105), while the Windows Start menu and the Linux applications menu offered *naudio Control* and their service units described "the naudio audio streaming daemon". Now the Start-menu shortcut (inside the package's *naudio* folder) and the desktop entry are **Network Audio Service**, Task Scheduler's description and `systemctl --user status` lead with it, and the page's quit dialog and *Runs at login* row say so — the row's Windows hint now names the `schtasks /Change … /ENABLE` or `/DISABLE` line for the state it shows, as the Linux hint names `systemctl --user`. The identifiers the commands take do not move: the task is still `\naudio\naudio-daemon`, the unit `naudio-daemon`, the package `naudio`, the bundle `org.kj5hst.naudio.control`. The name is owned in one place (`NAUDIO_APP_NAME` in `tools/CMakeLists.txt`), and the service-unit test asserts each platform's generated file leads with it — as a relation, with a control that replaces the name in a copy and must fail — so a rename that reaches one platform and not another fails there; the release pipeline's installer gates read the same off the installed shortcut, task, desktop entry and unit. The daemon's silence hint on Windows also names *Settings › Privacy & security › Microphone* beside the muted-input and wrong-device checks; whether a denial there arrives as silence, as macOS's does, is not measured, so the pane is named as a thing to check. `INSTALL.md`, the man page and the getting-started guide say the one name.
- **The control page is two tabs, Setup and Stream, so that configuring first is the page's shape** (issue #106). The two stacked panels let a stream run on settings the operator had changed but not saved, with the only cue a Save button two panels down, and a save while streaming was acknowledged in a notice on the other panel. Now Setup — the device pickers, every setting, Save, and the *Runs at login* row — comes first, and the page opens on it until a capture device has been chosen; Stream holds the status, the counters and Start / Stop / Restart / Quit. Start stays off, with the reason beside it and a link to Setup, until a capture device is chosen and saved, and while Setup holds unsaved changes (its tab carries a dot). Settings saved while a stream is running are announced on the Stream tab, beside a Restart button promoted to the primary action, and the notice clears when the stream restarts or stops. Nothing changed in the daemon or its API.
- **The macOS background service runs at your next login — including on a Mac upgraded from rc8 or earlier that never switched it on — and the Login Items switch is its one switch** (issue #102, second part). The launchd agent no longer ships `Disabled: true`. That key was the service's inertness, and it produced the bug reported: Login Items showed the agent's switch ON while launchd had never bootstrapped it, and `launchctl enable`/`disable` — the documented switch — turned out not to move the Login Items switch in either direction (measured: the two are independent stores). So the platform's switch is the switch: the agent ships on, and from the next login it serves the control page on `127.0.0.1`, opening no device until Start is pressed; turning it off in *System Settings › General › Login Items & Extensions* stops it and keeps it off. Installing still starts no capture — that guarantee moved from a key to the agent's own arguments (`--mode control`, autostart off), and the service-unit test now runs those arguments and requires an idle pipeline, as does the release pipeline's installer gate on the installed binary. The postinstall clears a `launchctl disable` left by the rc7/rc8 instructions, which would otherwise block the service while the switch said on. `INSTALL.md` and the man page describe the switch instead of the commands. Linux and Windows are unchanged: they have no such system switch and still arrive switched off with their commands.
- **On a fresh install, the macOS background service is listed in Login Items as Network Audio Service, not under the developer's name — and the Applications entry that opens the control page is now `Network Audio Service.app`** (issue #102, first part). Login Items names a launchd agent after the app its plist associates it with, so the launchd agent now names that bundle through `AssociatedBundleIdentifiers`, and the bundle is named after the service rather than the page because that row is the service's switch: the name in Settings, in the Finder and in Spotlight is one string (the Finder shows a bundle's on-disk name and ignores a display name that differs from it — measured — so the bundle is renamed on disk, not just relabelled). The release pipeline signs the bundle with the same Developer ID as the daemon: Background Task Management honours the association only when the app and the agent carry the same Team ID, and silently drops it otherwise (measured), so an unsigned build keeps the old grouping. Because the bundle is assembled by the installer's postinstall rather than shipped whole, its signature travels inside the `.pkg` as loose files beside the bundle's two pieces and is placed with them; the installer gate and the payload-signature step verify the assembled bundle on the installed files, and the postinstall removes the `naudio Control.app` that rc7 and rc8 placed, since two bundles with one identifier would leave Login Items to name either. A Mac that already had the service registered by an earlier release keeps the developer's name: Background Task Management names a legacy agent's entry once, when it first creates it, and neither reinstalling over it nor a reboot renames it (measured on an rc7 upgrade — the association is recorded and the app is recognised, but the entry's parent does not move). What does rename it, also measured, is making it drop the entry first: remove the agent's plist, open Login Items (the pane rescans and drops the orphaned entry), and run the installer again — no logout needed; `INSTALL.md` and the man page give the steps. The Linux and Windows shortcuts kept the page's name, **naudio Control**, until the first entry under *Changed* gave them the service's. The name is the issue's first half; the switch beside it is the entry above.
...
- **`na_c_inject_tone`, a sending client (`examples/c/inject_tone.c`).** It connects from the hardware-free NULL backend, reads the server's format back and injects a 1 kHz sawtooth as TX audio for N seconds, so the TX path — `na_audio_source --playback-id`, or a bridge's tx callback — is proved with no digital-mode application in the loop, and a silent bench splits into a naudio-side or an application-side fault. Sending is gated by the duplex mode, which is not PTT; it keys nothing. Built with the examples and driven by the new `naudio_tx_owner_line` test arm (the test-tone server names it owner, stamped, and releases it at disconnect; a client that connects and sends nothing is named no owner), not shipped in the packages.
...
- **Binary packages** ([#93](https://github.com/KJ5HST-LABS/naudio/issues/93)). `cmake --install` now
  ...
  A Homebrew tap ([`KJ5HST-LABS/homebrew-naudio`](https://github.com/KJ5HST-LABS/homebrew-naudio))
