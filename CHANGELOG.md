...
- **On a fresh install, the macOS background service is listed in Login Items as naudio Control, not under the developer's name** (issue #102, first part). The launchd agent now names `naudio Control.app` through `AssociatedBundleIdentifiers`, and the release pipeline signs that bundle with the same Developer ID as the daemon: Background Task Management honours the association only when the app and the agent carry the same Team ID, and silently drops it otherwise (measured), so an unsigned build keeps the old grouping. Because the bundle is assembled by the installer's postinstall rather than shipped whole, its signature travels inside the `.pkg` as loose files beside the bundle's two pieces and is placed with them; the installer gate and the payload-signature step verify the assembled bundle on the installed files. A Mac that already had the service registered by an earlier release keeps the developer's name: Background Task Management names a legacy agent's entry once, when it first creates it, and neither reinstalling the agent nor a reboot renames it (measured on an rc7 upgrade — the association is recorded and the app is recognised, but the entry's parent does not move). The name is the issue's first half; the switch beside it is the entry above.
